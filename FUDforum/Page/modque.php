 ...
 * $Id: modque.php.t 5963 2016-06-30 07:00:44Z naudefj $
 *
...
if (!_uid) {
    ...
    if (!($usr->users_opt & (1048576 | 524288))) {    // NOT is_admin OR is_mod.
        std_error('perms');
    }
}
...
if ($appr || $del || $delall) {
    if (!q_singleval(
        'SELECT CASE WHEN (' .
        q_bitand($usr->users_opt, 1048576) .
        ' = 0) THEN mm.id ELSE 1 END FROM fud30_msg m INNER JOIN fud30_thread t ON m.thread_id=t.id LEFT JOIN fud30_mod mm ON t.forum_id=mm.forum_id AND mm.user_id=' .
        _uid .
        ' WHERE m.id=' .
        ($appr ? $appr : ($del ? $del : $delall))
    )) {
        if (db_affected()) {
            std_error('perms');
        } else {
...
$r = q(
	...
	ORDER BY v.id, m.post_stamp DESC',
        $POSTS_PER_PAGE
    )
);
...
while ($obj = db_rowobj($r)) {
	...
	<td>
		...
		[<a href="/index.php?t=modque&amp;appr=' . $obj->id . '&amp;' . _rsid . '&amp;SQ=' . $GLOBALS['sq'] . '">Approve</a>]&nbsp;
		[<a href="/index.php?t=modque&amp;del=' . $obj->id . '&amp;' . _rsid . '&amp;SQ=' . $GLOBALS['sq'] . '">Delete</a>]&nbsp;
		[<a href="/index.php?t=modque&amp;delall=' . $obj->id . '&amp;' . _rsid . '&amp;SQ=' . $GLOBALS['sq'] . '">Delete all from user</a>]
	</td>
</tr>
...
F()->response->modqueMessage = $modque_message;
