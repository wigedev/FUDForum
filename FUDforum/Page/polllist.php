 ...
 * $Id: polllist.php.t 6078 2017-09-25 14:57:31Z naudefj $
 *
...
while ($obj = db_rowobj($c)) {
    ...
    if ($obj->owner && (!($obj->users_opt & 32768) || $is_a) && $FUD_OPT_2 & 32) {
        $online_indicator = $obj->last_visit > __request_timestamp__ ? '<img src="/theme/twig/images/online.png" title="' . $obj->login . ' is currently online" alt="' . $obj->login . ' is currently online" />&nbsp;' : '<img src="/theme/twig/images/offline.png" title="' . $obj->login . ' is currently offline" alt="' . $obj->login . ' is currently offline" />&nbsp;';
    } else {
        $online_indicator = '';
    }
    F()->response->pollEntries .= '<tr class="' . alt_var('search_alt', 'RowStyleA', 'RowStyleB') . '">
	<td class="wa">' . $obj->name . '</td>
	<td class="nw">' . strftime('%a, %d %B %Y %H:%M', $obj->creation_date) . '</td>
	<td class="nw">' . $online_indicator . '<a href="/index.php?t=usrinfo&amp;id=' . $obj->owner . '&amp;' . _rsid . '">' . htmlspecialchars($obj->alias, null, null, false) . '</a></td>
...
if ($ttl > $POLLS_PER_PAGE) {
    if ($FUD_OPT_2 & 32768) {
        F()->response->pager = tmpl_create_pager($start, $POLLS_PER_PAGE, $ttl, '/index.php/pl/' . $uid . '/', '/' . $oby . '/' . _rsid);
    } else {
        F()->response->pager = tmpl_create_pager($start, $POLLS_PER_PAGE, $ttl, '/index.php?t=polllist&amp;oby=' . $oby . '&amp;uid=' . $uid);
    }
} else if (!$ttl) {
    F()->response->pollEntries = '<tr>
	<td colspan="5" class="ac">There are no accessible polls.</td>
</tr>';
}

F()->response->start = $start;
F()->response->obyRevVal = $oby_rev_val;
...
<div class="content">
	...
	' . (__fud_real_user__ ? '<li><a href="/index.php?t=login&amp;' . _rsid . '&amp;logout=1&amp;SQ=' . $GLOBALS['sq'] . '" title="Logout"><img src="/theme/twig/images/top_logout.png" alt="" /> Logout [ ' . htmlspecialchars($usr->alias, null, null, false) . ' ]</a></li>' : '<li><a href="/index.php?t=login&amp;' . _rsid . '" title="Login"><img src="/theme/twig/images/top_login.png" alt="" /> Login</a></li>'); ?>
                ...
                <table cellspacing="1" cellpadding="2" class="ContentTable">
                    <tr>
                        <th>Poll Name</th>
                        <th class="nw"><a class="thLnk"
                                href="/index.php?t=polllist&amp;start=<?php echo $start; ?>&amp;oby=<?php echo $oby_rev_val; ?>&amp;<?php echo _rsid; ?>">Created
                                On</a></th>
