...
while ($obj = db_rowobj($c)) {
    switch ($obj->fldr) {
        case 1:
        case 2:
            $action = '<a href="/index.php?t=ppost&amp;' . _rsid . '&amp;reply=' . $obj->id . '"><img src="/theme/twig/images/msg_reply.gif" alt="" /></a>&nbsp;<a href="/index.php?t=ppost&amp;quote=' . $obj->id . '&amp;' . _rsid . '"><img src="/theme/twig/images/msg_quote.gif" alt="" /></a>&nbsp;<a href="/index.php?t=ppost&amp;forward=' . $obj->id . '&amp;' . _rsid . '"><img src="/theme/twig/images/msg_forward.gif" alt="" /></a>';
            break;
        case 3:
            ...
            break;
        case 5:
            $action = '<a href="/index.php?t=ppost&amp;' . _rsid . '&amp;forward=' . $obj->id . '"><img src="/theme/twig/images/msg_forward.gif" alt="" /></a>';
            break;
        case 4:
            $action = '<a href="/index.php?t=ppost&amp;' . _rsid . '&amp;msg_id=' . $obj->id . '"><img src="/theme/twig/images/msg_edit.gif" alt="" /></a>';
            break;
    }
    ...
    if ($FUD_OPT_2 & 32 && (!($obj->users_opt & 32768) || $is_a)) {
        ...
        if (($obj->time_sec + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) {
            $online_indicator = '<img src="/theme/twig/images/online.png" alt="' . $obj->login . ' is currently online" title="' . $obj->login . ' is currently online" />&nbsp;';
        } else {
            $online_indicator = '<img src="/theme/twig/images/offline.png" alt="' . $obj->login . ' is currently offline" title="' . $obj->login . ' is currently offline" />&nbsp;';
        }
    } else {
        $online_indicator = '';
    }
	...
	<td>' . ($obj->read_stamp ? '<img src="/theme/twig/images/pmsg_read.png" alt="Read private message" title="Read private message" />' : '<img src="/theme/twig/images/pmsg_unread.png" alt="Unread private message" title="Unread private message" />') . '</td>
...
if ($FUD_OPT_2 & 32768) {
    F()->response->pagePager = tmpl_create_pager($start, $count, $ttl, '/index.php/pdm/' . $folder_id . '/0/' . strtolower($s[0]) . '/' . $o . '/', '/' . _rsid);
} else {
    F()->response->pagePager = tmpl_create_pager($start, $count, $ttl, '/index.php?t=pmsg&amp;s=' . strtolower($s[0]) . '&amp;o=' . $o . '&amp;folder_id=' . $folder_id . '&amp;' . _rsid);
}

F()->response->tabs = $tabs;
F()->response->currentPageTemplate = tmpl_cur_ppage($folder_id, $folders);
F()->response->percentFull = $percent_full;
F()->response->fullIndicator = $full_indicator;
F()->response->privateTools = $private_tools;
F()->response->selectOptionsCurFolder = $select_options_cur_folder;
F()->response->folderId = $folder_id;
F()->response->o = $o;
F()->response->s = $s;
F()->response->subjectOrder = $o=='p.subject' && $s=='ASC' ? 'd' : 'a';
F()->response->folderAlias = $folder_id == 3 ? 'alias2' : 'alias';
F()->response->folderOrder = ($o=='u.alias' || $o=='u2.alias') && $s=='ASC' ? 'd' : 'a';
F()->response->privateMsgEntry = $private_msg_entry;
F()->response->privateTools = $private_tools;
