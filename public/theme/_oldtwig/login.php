...
function make_alias($text)
{
    ...
    return char_fix(str_replace([']', '['], ['&#93;', '&#91;'], htmlspecialchars($text)));
}
...
class fud_user_reg extends fud_user
{
    ...
    {
        foreach (['name', 'location', 'occupation', 'interests', 'bio'] as $v) {
            if ($this->{$v}) {
    ...
    function add()
    {
        ...
        if (empty($this->theme)) {
            $this->theme = q_singleval(
                q_limit('SELECT id FROM fud30_themes WHERE theme_opt>=2 AND ' . q_bitand('theme_opt', 2) . ' > 0', 1)
            );
        }
    ...
    function sync()
    {
        if (!empty($this->plaintext_passwd)) {
            ...
            $passwd = 'passwd=\'' .
                sha1($this->salt . sha1($this->plaintext_passwd)) .
                '\', salt=\'' .
                $this->salt .
                '\', ';
        } else {
        ...
        $rb_mod_list = (!($this->users_opt & 524288) &&
            ($is_mod = q_singleval('SELECT id FROM fud30_mod WHERE user_id=' . $this->id)) &&
            (q_singleval('SELECT alias FROM fud30_users WHERE id=' . $this->id) == $this->alias));

        $this->html_fields();

        q(
            'UPDATE fud30_users SET ' . $passwd . '
			name=' . _esc($this->name) . ',
...
function usr_email_unconfirm($id)
{
    ...
    q(
        'UPDATE fud30_users SET users_opt=' .
        q_bitand('users_opt', ~131072) .
        ', conf_key=\'' .
        $conf_key .
        '\' WHERE id=' .
        $id
...
function user_login($id, $cur_ses_id, $use_cookies)
{
    ...
    if ($GLOBALS['FUD_OPT_2'] & 256 && ($s = db_saq('SELECT ses_id, sys_id FROM fud30_ses WHERE user_id=' . $id))) {
        if ($use_cookies) {
            setcookie(
                $GLOBALS['COOKIE_NAME'],
                $s[0],
                __request_timestamp__ + $GLOBALS['COOKIE_TIMEOUT'],
                $GLOBALS['COOKIE_PATH'],
                $GLOBALS['COOKIE_DOMAIN']
            );
        }
    ...
    q(
        'UPDATE fud30_users SET last_used_ip=\'' .
        get_ip() .
        '\', ' .
        $flag .
        ' sq=\'' .
...
function rebuildmodlist()
{
    ...
    $c = uq(
        'SELECT u.id, u.alias, f.id FROM ' .
        $tbl .
        'mod mm INNER JOIN ' .
        $tbl .
        'users u ON mm.user_id=u.id INNER JOIN ' .
        $tbl .
        'forum f ON f.id=mm.forum_id ORDER BY f.id,u.alias'
    );
    $u = $ar = [];
    ...
    q(
        ...
        $tbl .
        'users SET users_opt=' .
        q_bitand('users_opt', ~524288) .
        ' WHERE users_opt>=524288 AND ' .
        q_bitand('users_opt', 524288) .
        '>0'
    );
    if ($u) {
...
function ret_flag($raw = 0)
{
    ...
    if ($GLOBALS['FUD_OPT_3'] & 524288) {    // ENABLE_GEO_LOCATION.
        $val = db_saq(
            'SELECT cc, country FROM fud30_geoip WHERE ' . sprintf('%u', ip2long($ip)) . ' BETWEEN ips AND ipe'
        );
        if ($raw) {
            return $val ? $val : [null, null];
        }
        if ($val) {
...
function is_allowed_user(&$usr, $simple = 0)
{
    ...
    if ($banned || is_email_blocked($usr->email) || is_login_blocked($usr->login) || is_ip_blocked(get_ip())) {
        ...
        setcookie(
            $GLOBALS['COOKIE_NAME'] . '1',
            'd34db33fd34db33fd34db33fd34db33f',
            ($ban_expiry ? $ban_expiry : (__request_timestamp__ + 63072000)),
            $GLOBALS['COOKIE_PATH'],
            $GLOBALS['COOKIE_DOMAIN']
        );
        if ($banned) {
            error_dialog(
                ...
                'Your account was ' .
                ($ban_expiry ? 'temporarily banned until ' .
                    strftime('%a, %d %B %Y %H:%M', $ban_expiry) : 'permanently banned') .
                ' from accessing the site, due to a violation of the forum&#39;s rules.
<br />
<br />
<span class="GenTextRed">' .
                $ban_reason .
                '</span>'
            );
        } else {
            error_dialog(
                'ERROR: Your account has been filtered out.',
                'Your account has been blocked from accessing the forum due to one of the installed user filters.'
            );
        }
    }
    ...
    if ($GLOBALS['FUD_OPT_1'] & 1048576 && $usr->users_opt & 262144) {
        error_dialog(
            ...
            'We have not received a confirmation from your parent and/or legal guardian, which would allow you to post messages. If you lost your COPPA form, <a href="/index.php?t=coppa_fax&amp;' .
            _rsid .
            '">view it again</a>.'
        );
    }
...
if ($FUD_OPT_2 & 1) {
    ...
    $list = db_all(
        'SELECT id FROM fud30_users WHERE ' .
        q_bitand('users_opt', 131072) .
        '=0 AND join_date<' .
        $account_expiry_date .
        ' AND posted_msg_count=0 AND last_visit<' .
        $account_expiry_date .
        ' AND id!=1 AND ' .
        q_bitand('users_opt', 1048576) .
        '=0'
...
if (!empty($_GET['logout']) && sq_check(0, $usr->sq)) {
    ...
    switch ($page) {
        ...
        default:
            if ($page == 'msg' || $page == 'tree') {
                if (empty($tmp['th'])) {
                    if (empty($tmp['goto']) || !q_singleval(
                            'SELECT t.forum_id
								FROM fud30_msg m
								INNER JOIN fud30_thread t ON m.thread_id=t.id
								INNER JOIN fud30_group_cache g ON g.user_id=0 AND g.resource_id=t.forum_id AND ' .
                            q_bitand('g.group_cache_opt', 2) .
                            ' > 0
								WHERE m.id=' .
                            (int)$tmp['goto']
                        )) {
                ...
                if ($page == 'thread' || $page == 'threadt') {
                    if (!q_singleval(
                        'SELECT id FROM fud30_group_cache WHERE user_id=0 AND resource_id=' .
                        (isset($tmp['frm_id']) ? (int)$tmp['frm_id'] : 0) .
                        ' AND ' .
    ...
    if ($FUD_OPT_2 & 32768 && $returnto && $returnto[0] == '/') {
        ...
        header('Location: ' . $GLOBALS['WWW_ROOT'] . '?' . str_replace(['?', '&&'], ['&', '&'], $returnto));
    }
    exit;
}
...
if ($usr_d || isset($_POST['login']) && !error_check()) {
    ...
    if (!$usr_d &&
        !($usr_d = db_sab(
            'SELECT last_login, id, passwd, salt, login, email, users_opt, ban_expiry, ban_reason FROM fud30_users WHERE login=' .
            _esc($_POST['login'])
        ))) {
        /* Cannot login: user not in DB. */
        login_php_set_err('login', 'Invalid login/password combination');
    } else {
        if (($usr_d->last_login + $MIN_TIME_BETWEEN_LOGIN) > __request_timestamp__) {
            /* Flood control. */
            q('UPDATE fud30_users SET last_login=' . __request_timestamp__ . ' WHERE id=' . $usr_d->id);
            login_php_set_err(
                'login',
                'This forum allows one login attempt every ' .
                $MIN_TIME_BETWEEN_LOGIN .
                ' seconds, please wait ' .
                ($usr_d->last_login + $MIN_TIME_BETWEEN_LOGIN - __request_timestamp__) .
                ' seconds before your next attempt.'
            );
            /* Check password: No salt -> old md5() auth; with salt -> new sha1() auth. */
        } else {
            if (!isset($usr_d->alias) && (empty($usr_d->salt) && $usr_d->passwd != md5($_POST['password']) ||
                ...
                logaction(
                    ...
                    0,
                    'Invalid ' .
                    ($usr_d->users_opt & 1048576 ? 'FORUM ADMIN ' : '') .
                    'password for login ' .
                    htmlspecialchars(_esc($_POST['login'])) .
                    ' from IP ' .
                    get_ip() .
                    '.'
                );
    ...
    if ($GLOBALS['_ERROR_'] != 1) {
        ...
        $ses_id = user_login(
            $usr_d->id,
            $usr->ses_id,
            ((empty($_POST['use_cookie']) && $FUD_OPT_1 & 128) ? false : true)
        );

        if (!($usr_d->users_opt & 131072)) {
            error_dialog(
                'ERROR: Your account is not yet confirmed',
                'You have not confirmed your account via e-mail yet.<br /><table border="0"><tr><td><ol><li>If you have not received a confirmation e-mail, <a href="/index.php?t=reset&amp;email=' .
                $usr_d->email .
                '&amp;S=' .
                $ses_id .
                '">request it again</a>.</li><li>If ' .
                $usr_d->email .
                ' is not your correct e-mail address, <a href="/index.php?t=register&amp;S=' .
                $ses_id .
                '">specify the correct address</a>.</li></ol></td></tr></table>',
                null,
                $ses_id
            );
        }
...
F()->response->resetLoginNotify = $reset_login_notify;
F()->response->loginError = login_php_get_err('login');
F()->response->passwordError = login_php_get_err('password');
F()->response->adm = (isset($_GET['adm']) ? '1' : ''); // TODO: Should this be stored in the session?
?>
<!DOCTYPE html>
...
<div class="content">
	...
	' .
                ...
                <form id="login" method="post" action="/index.php?t=login"<?php echo ($FUD_OPT_3 &
...
<tr>
	<td class="RowStyleA GenText" colspan="3">
        ...
        <tr class="RowStyleB">
            ...
            <td class="nw"><?php echo($FUD_OPT_1 & 2 ? '<a href="/index.php?t=register&amp;' .
                    _rsid .
                    '">Want to register?</a>' : ''); ?></td>
        </tr>
        <tr class="RowStyleA">
            <td class="GenText">Password:</td>
            <td><?php echo login_php_get_err('password'); ?><input type="password" tabindex="2" name="password"/></td>
            <td class="nw"><?php echo($FUD_OPT_4 & 2 ? '<a href="/index.php?t=reset&amp;' .
                    _rsid .
...
</div><?php echo _hs; ?><input type="hidden" name="adm" value="<?php echo(isset($_GET['adm']) ? '1' : ''); ?>"/></form>
<br/>
