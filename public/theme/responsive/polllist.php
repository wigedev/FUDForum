...
if (_uid) {
	...
	while ($obj = db_rowobj($c)) {
		...
		if ($obj->owner && (!($obj->users_opt & 32768) || $is_a) && $FUD_OPT_2 & 32) {
			$online_indicator = $obj->last_visit > __request_timestamp__ ? '<img src="/theme/responsive/images/online.png" title="'.$obj->login.' is currently online" alt="'.$obj->login.' is currently online" />&nbsp;' : '<img src="/theme/responsive/images/offline.png" title="'.$obj->login.' is currently offline" alt="'.$obj->login.' is currently offline" />&nbsp;';
		} else {
			$online_indicator = '';
		}
