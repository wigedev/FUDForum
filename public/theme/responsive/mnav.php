...
<table cellspacing="0" cellpadding="0" class="ContentTable">';
		while ($r = db_rowobj($c)) {
			if ($oldf != $r->forum_id) {
				$mnav_data .= '<tr><th colspan="3"> Forum: <a class="thLnk" href="/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'"><span class="lg">'.htmlspecialchars($r->forum_name, null, null, false).'</span></a></th></tr>';
				$oldf = $r->forum_id;
			}
			if ($oldt != $r->thread_id) {
	...
	<td>
		...
		<tr class="mnavH">
			...
			<td class="TopBy wa ac">Posted By: '.(!empty($r->poster_id) ? '<a href="/index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.htmlspecialchars($r->alias, null, null, false).'</a>' : $GLOBALS['ANON_NICK'].'' ) .'</td>
