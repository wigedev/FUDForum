...
while ($r = db_rowobj($c)) {
    ...
    if ($r->reply_to > 0) {
        if (!isset($arr[$r->reply_to]->kiddie_count)) {
            $arr[$r->reply_to]->kiddie_count = 0;
        }
...
F()->response->forumPath = draw_forum_path($frm->cat_id, $frm->name, $frm->forum_id, $frm->subject);
F()->response->getSrch = $_GET['srch']; // TODO: Move to request
F()->response->mod = $MOD;
F()->response->voteCount = convertPlural($frm->n_rating, array('' . $frm->n_rating . ' vote', '' . $frm->n_rating . ' votes'));
F()->response->permissions = $perms;
F()->response->lockThread = $lock_thread;
F()->response->subscribeStatus = $subscribe_status;
F()->response->bookmarkStatus = $bookmark_status;
F()->response->mid = $mid;
F()->response->tmplDrawMsg = tmpl_drawmsg($msg_obj, $usr, $perms, false, $n, array($prev_msg, $next_msg));
F()->response->treeData = $tree_data;
F()->response->prevThreadLink = $prev_thread_link;
F()->response->nextThreadLink = $next_thread_link;
F()->response->forumSelect = tmpl_create_forum_select((isset($frm->forum_id) ? $frm->forum_id : $frm->id), $usr->users_opt & 1048576);
