...
$c = uq(
    'SELECT u.alias, k.rating, k.id, k.msg_id FROM fud30_karma_rate_track k INNER JOIN fud30_users u ON k.user_id = u.id WHERE k.poster_id = ' .
    $usrid->poster_id
);
$table_data = '';
while ($r = db_rowarr($c)) {
	...
	<td>' .
        $r[1] .
        '</td>
	<td><a href="/index.php?t=msg&amp;goto=' .
        $r[3] .
        '#msg_' .
        $r[3] .
        '" target="_blank">' .
        $r[3] .
        '</a></td>
	<td><a href="/index.php?t=karma_track&amp;msgid=' .
        $r[3] .
        '&amp;karmaid=' .
        $r[2] .
        '&amp;' .
