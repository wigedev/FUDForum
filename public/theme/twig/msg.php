...
function register_vote(&$options, $poll_id, $opt_id, $mid)
{
    ...
    if (!isset($options[$opt_id]) ||
        q_singleval('SELECT id FROM fud30_poll_opt_track WHERE poll_id=' . $poll_id . ' AND user_id=' . _uid)) {
        return;
    }

    if (db_li(
        'INSERT INTO fud30_poll_opt_track(poll_id, user_id, ip_addr, poll_opt) VALUES(' .
        $poll_id .
        ', ' .
        _uid .
        ', ' .
        (!_uid ? _esc(get_ip()) : 'null') .
        ', ' .
        $opt_id .
        ')',
        $a
...
function tmpl_drawmsg($obj, $usr, $perms, $hide_controls, &$m_num, $misc)
{
    ...
    if (!$hide_controls && $misc) {
        ...
        if ($_GET['t'] == 'tree' || $_GET['t'] == 'tree_msg') {
            $prev_message = $misc[0] ? '<a href="javascript://" onclick="fud_tree_msg_focus(' .
                $misc[0] .
                ', \'' .
                s .
                '\', \'utf-8\'); return false;"><img src="/theme/twig/images/up.png" title="Go to previous message" alt="Go to previous message" width="16" height="11" /></a>' : '';
            $next_message = $misc[1] ? '<a href="javascript://" onclick="fud_tree_msg_focus(' .
                $misc[1] .
                ', \'' .
                s .
                '\', \'utf-8\'); return false;"><img alt="Go to previous message" title="Go to next message" src="/theme/twig/images/down.png" width="16" height="11" /></a>' : '';
        } else {
            /* Handle previous link. */
            if (!$m_num && $obj->id > $obj->root_msg_id) { /* prev link on different page */
                $prev_message = '<a href="/index.php?t=' .
                    $_GET['t'] .
                    '&amp;' .
                    _rsid .
                    '&amp;prevloaded=1&amp;th=' .
                    ...
                    reveal_lnk .
                    unignore_tmp .
                    '"><img src="/theme/twig/images/up.png" title="Go to previous message" alt="Go to previous message" width="16" height="11" /></a>';
            } else {
                if ($m_num) { /* Inline link, same page. */
                    $prev_message = '<a href="javascript://" onclick="chng_focus(\'#msg_num_' .
                        $m_num .
                        '\');"><img alt="Go to previous message" title="Go to previous message" src="/theme/twig/images/up.png" width="16" height="11" /></a>';
                }
            }
            ...
            if ($obj->id < $obj->last_post_id) {
                if ($m_num && !($misc[1] - $m_num - 1)) { /* next page link */
                    $next_message = '<a href="/index.php?t=' .
                        $_GET['t'] .
                        '&amp;' .
                        _rsid .
                        '&amp;prevloaded=1&amp;th=' .
                        ...
                        reveal_lnk .
                        unignore_tmp .
                        '"><img alt="Go to previous message" title="Go to next message" src="/theme/twig/images/down.png" width="16" height="11" /></a>';
                    $next_page = '<a href="/index.php?t=' .
                        $_GET['t'] .
                        '&amp;' .
                        _rsid .
                        '&amp;prevloaded=1&amp;th=' .
                        ...
                        reveal_lnk .
                        unignore_tmp .
                        '">Next Page <img src="/theme/twig/images/goto.gif" alt="" /></a>';
                } else {
                    $next_message = '<a href="javascript://" onclick="chng_focus(\'#msg_num_' .
                        ($m_num + 2) .
                        '\');"><img alt="Go to next message" title="Go to next message" src="/theme/twig/images/down.png" width="16" height="11" /></a>';
                }
            }
        }
        ++$m_num;
	...
	<td>
		...
		<tr>
            ...
            '"></a>
				' .
            ($obj->user_id ? 'Message by <a href="/index.php?t=usrinfo&amp;' .
                _rsid .
                '&amp;id=' .
                $obj->user_id .
                '">' .
                $obj->login .
                '</a> is ignored' : $GLOBALS['ANON_NICK'] . ' is ignored') .
            '&nbsp;
				[<a href="/index.php?' .
            make_reveal_link($obj->id) .
            '">reveal message</a>]&nbsp;
				[<a href="/index.php?' .
            make_tmp_unignore_lnk($obj->poster_id) .
            '">reveal all messages by ' .
            $user_login .
            '</a>]&nbsp;
				[<a href="/index.php?t=ignore_list&amp;del=' .
            $obj->poster_id .
            '&amp;redr=1&amp;' .
            _rsid .
            '&amp;SQ=' .
            $GLOBALS['sq'] .
            '">stop ignoring this user</a>]</td>
				<td class="MsgIg" align="right">' .
            $prev_message .
            $next_message .
            '
			</td>
    ...
    if ($obj->user_id && !$hide_controls) {
        ...
        $avatar = ($obj->avatar_loc || $level_image) ? '<td class="avatarPad wo">' .
            $obj->avatar_loc .
            $level_image .
            '</td>' : '';
        ...
        if (($o2 & 32 && !($a & 32768)) || $b & 1048576) {
            $online_indicator = (($obj->time_sec + $GLOBALS['LOGEDIN_TIMEOUT'] * 60) >
                __request_timestamp__) ? '<img src="/theme/twig/images/online.png" alt="' .
                $obj->login .
                ' is currently online" title="' .
                $obj->login .
                ' is currently online" />&nbsp;' : '<img src="/theme/twig/images/offline.png" alt="' .
                $obj->login .
                ' is currently offline" title="' .
                $obj->login .
        ...
        $user_link = '<a href="/index.php?t=usrinfo&amp;id=' .
            $obj->user_id .
            '&amp;' .
            _rsid .
            '">' .
        ...
        $location = $obj->location ? '<br /><b>Location: </b>' .
            (strlen($obj->location) > $GLOBALS['MAX_LOCATION_SHOW'] ? substr(
                    $obj->location,
                    0,
                    $GLOBALS['MAX_LOCATION_SHOW']
                ) . '...' : $obj->location) : '';

        if (_uid && _uid != $obj->user_id) {
            $buddy_link = !isset($usr->buddy_list[$obj->user_id]) ? '<a href="/index.php?t=buddy_list&amp;add=' .
                $obj->user_id .
                '&amp;' .
                _rsid .
                '&amp;SQ=' .
                $GLOBALS['sq'] .
                '">add to buddy list</a><br />' : '<a href="/index.php?t=buddy_list&amp;del=' .
                $obj->user_id .
                '&amp;redr=1&amp;' .
                _rsid .
                '&amp;SQ=' .
                $GLOBALS['sq'] .
                '">remove from buddy list</a><br />';
            $ignore_link = !isset($usr->ignore_list[$obj->user_id]) ? '<a href="/index.php?t=ignore_list&amp;add=' .
                $obj->user_id .
                '&amp;' .
                _rsid .
                '&amp;SQ=' .
                $GLOBALS['sq'] .
                '">ignore all messages by this user</a>' : '<a href="/index.php?t=ignore_list&amp;del=' .
                $obj->user_id .
                '&amp;redr=1&amp;' .
                _rsid .
                '&amp;SQ=' .
                $GLOBALS['sq'] .
                '">stop ignoring messages by this user</a>';
        ...
        if ($b & 16384) {
            $im = '';
            if ($obj->icq) {
                $im .= '<a href="/index.php?t=usrinfo&amp;id=' .
                    $obj->poster_id .
                    '&amp;' .
                    _rsid .
                    '#icq_msg"><img title="' .
                    $obj->icq .
                    '" src="/theme/twig/images/icq.png" alt="" /></a>';
            }
            if ($obj->aim) {
                $im .= '<a href="aim:goim?screenname=' .
                    $obj->aim .
                    '&amp;message=Hi.+Are+you+there?"><img alt="" src="/theme/twig/images/aim.png" title="' .
                    $obj->aim .
                    '" /></a>';
            }
            if ($obj->yahoo) {
                $im .= '<a href="http://edit.yahoo.com/config/send_webmesg?.target=' .
                    $obj->yahoo .
                    '&amp;.src=pg"><img alt="" src="/theme/twig/images/yahoo.png" title="' .
                    $obj->yahoo .
                    '" /></a>';
            }
            if ($obj->msnm) {
                $im .= '<a href="mailto: ' .
                    $obj->msnm .
                    '"><img alt="" src="/theme/twig/images/msnm.png" title="' .
                    $obj->msnm .
                    '" /></a>';
            }
            if ($obj->jabber) {
            ...
            if ($obj->skype) {
                $im .= '<a href="callto://' .
                    $obj->skype .
                    '"><img src="/theme/twig/images/skype.png" title="' .
                    $obj->skype .
                    '" alt="" /></a>';
            }
            if ($obj->twitter) {
                $im .= '<a href="http://twitter.com/' .
                    $obj->twitter .
                    '"><img src="/theme/twig/images/twitter.png" title="' .
                    $obj->twitter .
                    '" alt="" /></a>';
            }
            if ($im) {
	 ...
	 */
    if (!$hide_controls &&
        $obj->message_threshold &&
        $obj->length_preview &&
        $obj->length > $obj->message_threshold &&
        !isset($GLOBALS['__FMDSP__'][$obj->id])) {
        $msg_body = '<span class="MsgBodyText">' .
            read_msg_body($obj->offset_preview, $obj->length_preview, $obj->file_id_preview) .
            '</span>
...<br /><br /><div class="ac">[ <a href="/index.php?' .
            make_reveal_link($obj->id) .
            '">Show the rest of the message</a> ]</div>';
    } else {
        if ($obj->length) {
            $msg_body = '<span class="MsgBodyText">' .
                read_msg_body($obj->foff, $obj->length, $obj->file_id) .
                '</span>';
        } else {
	...
	<img alt="" src="/images/mime/' .
                    $v[4] .
                    '" class="at" />
	<span class="GenText fb">Attachment:</span> <a href="/index.php?t=getfile&amp;id=' .
                    $v[0] .
                    '&amp;' .
                    _rsid .
                    '" title="' .
	...
	<span class="SmallText">(Size: ' .
                    ($sz < 1000 ? number_format($sz, 2) . 'KB' : number_format($sz / 1024, 2) . 'MB') .
                    ', Downloaded ' .
        ...
        if (_uid) {
            if ($o1 & 128 && !isset($_COOKIE[$GLOBALS['COOKIE_NAME']])) {
                $msg_body = str_replace(
                    '<img src="index.php?t=getfile',
                    '<img src="index.php?t=getfile&amp;S=' . s,
                    $msg_body
                );
                $tap = 1;
            }
    ...
    /* Handle poll votes. */
    if (!empty($_POST['poll_opt']) &&
        ($_POST['poll_opt'] = (int)$_POST['poll_opt']) &&
        !($obj->thread_opt & 1) &&
        $perms & 512) {
    ...
    if ($obj->poll_id && $obj->poll_cache) {
        ...
        foreach ($obj->poll_cache as $k => $v) {
            ...
            if ($show_res) {
                ...
                $poll_data .= '<tr class="' .
                    alt_var('msg_poll_alt_clr', 'RowStyleB', 'RowStyleA') .
                    '">
	<td>' .
                    $i .
                    '.</td>
	<td>' .
                    $v[0] .
                    '</td>
	<td><img src="/theme/twig/images/poll_pix.gif" alt="" height="10" width="' .
                    $length .
                    '" /> ' .
                    $v[1] .
                    ' / ' .
...
<tr>
	<th class="nw" colspan="3">' .
                $obj->poll_name .
                '<span class="ptp">[ ' .
                $obj->total_votes .
                ' ' .
                convertPlural($obj->total_votes, ['vote', 'votes']) .
                ' ]</span></th>
</tr>
' .
                $poll_data .
...
<input type="hidden" name="pl_view" value="' .
                $obj->poll_id .
                '" />
</form>
<br />';
...
<br />';
    ...
    if ($obj->update_stamp) {
        if ($obj->updated_by != $obj->poster_id && $o1 & 67108864) {
            $modified_message = '<p class="fl">[Updated on: ' .
                strftime('%a, %d %B %Y %H:%M', $obj->update_stamp) .
                '] by Moderator</p>';
        } else {
    ...
    if (!$hide_controls) {
        ...
        if ($o2 & 536870912) {
            if ($obj->reply_to && $obj->reply_to != $obj->id) {
                $rpl = '<span class="SmallText">[<a href="/index.php?t=' .
                    $lnk .
                    '&amp;th=' .
                    $obj->thread_id .
                    '&amp;goto=' .
                    ...
                    '">message #' .
                    $obj->id .
                    '</a> is a reply to <a href="/index.php?t=' .
                    $lnk .
                    '&amp;th=' .
                    $obj->thread_id .
                    '&amp;goto=' .
        ...
        if (_uid &&
            ...
            $edit_link = '<a href="/index.php?t=post&amp;msg_id=' .
                $obj->id .
                '&amp;' .
                _rsid .
                '"><img alt="" src="/theme/twig/images/msg_edit.gif" /></a>&nbsp;&nbsp;&nbsp;&nbsp;';
        } else {
            $edit_link = '';
        }

        if (!($obj->thread_opt & 1) || $perms & 4096) {
            $reply_link = '<a href="/index.php?t=post&amp;reply_to=' .
                $obj->id .
                '&amp;' .
                _rsid .
                '"><img alt="" src="/theme/twig/images/msg_reply.gif" /></a>&nbsp;';
            $quote_link = '<a href="/index.php?t=post&amp;reply_to=' .
                $obj->id .
                '&amp;quote=true&amp;' .
                _rsid .
                '"><img alt="" src="/theme/twig/images/msg_quote.gif" /></a>';
        } else {
	...
	<td class="MsgSpacer">
		...
		<tr>
			<td class="MsgR1 vt al expanded"><a name="msg_num_' .
        $m_num .
        '"></a><a name="msg_' .
        $obj->id .
        '"></a>' .
        ($obj->icon && !$hide_controls ? '<img src="/images/message_icons/' .
            $obj->icon .
            '" alt="' .
            $obj->icon .
            '" />&nbsp;&nbsp;' : '') .
        '<span class="MsgSubText"><a href="/index.php?t=' .
        $lnk .
        '&amp;th=' .
        $obj->thread_id .
        '&amp;goto=' .
        ...
        $rpl .
        '</td>
			<td class="MsgR1 vt ar"><span class="DateText">' .
        ...
        '
						' .
        (!$hide_controls ? ($obj->disp_flag_cc &&
            $GLOBALS['FUD_OPT_3'] & 524288 ? '&nbsp;&nbsp;<img src="/images/flags/' .
                $obj->disp_flag_cc .
                '.png" border="0" width="16" height="11" title="' .
                $obj->flag_country .
                '" alt="' .
                $obj->flag_country .
                '"/>' : '') .
            ($obj->user_id ? '<br /><b>Messages:</b> ' .
                $obj->posted_msg_count .
                '<br /><b>Registered:</b> ' .
                strftime('%B %Y', $obj->join_date) .
                ' ' .
                $location : '') : '') .
        '
						' .
        ($GLOBALS['FUD_OPT_4'] & 4 && $obj->poster_id > 0 ? '<div class="karma_usr_' .
            $obj->poster_id .
            ' SmallText">
' .
            ($MOD ? '<a href="javascript://" onclick="window_open(\'/index.php?t=karma_track&amp;' .
                _rsid .
                '&amp;msgid=' .
                $obj->id .
                '\', \'karma_rating_track\', 300, 400);" class="karma">' : '') .
            '
	<b>Karma:</b> ' .
            $obj->karma .
            '
...
' : '') .
        ...
        $dmsg_bd_il .
        $dmsg_im_row .
        (!$hide_controls ? (($obj->host_name && $o1 & 268435456) ? '<b>From:</b> ' . $obj->host_name . '<br />' : '') .
            (($b & 1048576 || $usr->md || $o1 & 134217728) ? '<b>IP:</b> <a href="/index.php?t=ip&amp;ip=' .
                $obj->ip_addr .
                '&amp;' .
                _rsid .
                '">' .
...
<tr>
	<td colspan="2" class="MsgR3">
        ...
        '
		' .
        (!$hide_controls ? (($obj->sig &&
                $o1 & 32768 &&
                $obj->msg_opt & 1 &&
                $b & 4096 &&
                !($a & 67108864)) ? '<br /><br /><div class="signature" />' . $obj->sig . '</div>' : '') .
            '
		<div class="SmallText clear">' .
            $modified_message .
            '<p class="fr"><a href="/index.php?t=report&amp;msg_id=' .
            $obj->id .
            '&amp;' .
            _rsid .
            '" rel="nofollow">Report message to a moderator</a></p>' : '') .
        '</div>
</td></tr>
' .
        (!$hide_controls ? '<tr>
	<td colspan="2" class="MsgToolBar">
		...
		<tr>
			<td class="al nw">
				' .
            ($obj->user_id ? '<a href="/index.php?t=usrinfo&amp;id=' .
                $obj->user_id .
                '&amp;' .
                _rsid .
                '"><img alt="" src="/theme/twig/images/msg_about.gif" /></a>&nbsp;' .
                (($o1 & 4194304 && $a & 16) ? '<a href="/index.php?t=email&amp;toi=' .
                    $obj->user_id .
                    '&amp;' .
                    _rsid .
                    '" rel="nofollow"><img alt="" src="/theme/twig/images/msg_email.gif" /></a>&nbsp;' : '') .
                ($o1 & 1024 ? '<a href="/index.php?t=ppost&amp;toi=' .
                    $obj->user_id .
                    '&amp;rmid=' .
                    $obj->id .
                    '&amp;' .
                    _rsid .
                    '"><img alt="Send a private message to this user" title="Send a private message to this user" src="/theme/twig/images/msg_pm.gif" /></a>' : '') : '') .
            '
				' .
            (($GLOBALS['FUD_OPT_4'] & 4 &&
                $perms & 1024 &&
                $obj->poster_id > 0 &&
                !$obj->cant_karma &&
                $obj->poster_id != $usr->id) ? '
    <span id=karma_link_' .
                $obj->id .
                ' class="SmallText">Rate author:
	<a href="javascript://" onclick="changeKarma(' .
                $obj->id .
                ',' .
                $obj->poster_id .
                ',\'up\',\'' .
	...
	<a href="javascript://" onclick="changeKarma(' .
                ...
                '\',\'' .
                $usr->sq .
                '\');" class="karma down">-1</a>
    </span>
' : '') .
            '
			</td>
			<td class="GenText wa ac">' .
            $next_page .
            '</td>
			<td class="nw ar">
				' .
            ($perms & 32 ? '<a href="/index.php?t=mmod&amp;del=' .
                $obj->id .
                '&amp;' .
                _rsid .
                '"><img alt="" src="/theme/twig/images/msg_delete.gif" /></a>&nbsp;' : '') .
            '
				' .
            $edit_link .
            '
...
function &get_all_read_perms($uid, $mod)
{
    ...
    if (_uid) {
        ...
        $r = uq(
            'SELECT resource_id FROM fud30_group_cache WHERE resource_id NOT IN (' .
            implode(',', array_keys($limit)) .
            ') AND user_id=2147483647 AND ' .
            q_bitand('group_cache_opt', 2) .
            ' > 0'
        );
        while ($ent = db_rowarr($r)) {
...
function make_perms_query(&$fields, &$join, $fid = '')
{
    ...
    if (_uid) {
        $join = ' INNER JOIN fud30_group_cache g1 ON g1.user_id=2147483647 AND g1.resource_id=' .
            $fid .
...
function get_prev_next_th_id($frm_id, $th, &$prev, &$next)
{
    ...
    $c = uq(
        'SELECT m.id, m.subject, tv.seq, t.moved_to FROM fud30_tv_' .
        $frm_id .
        ' tv INNER JOIN fud30_thread t ON tv.thread_id=t.id INNER JOIN fud30_msg m ON t.root_msg_id=m.id WHERE tv.seq IN(' .
        ($id - 1) .
        ', ' .
        ($id + 1) .
        ')'
	...
	<td class="GenText al"><a href="/index.php?t=' .
                $_GET['t'] .
                '&amp;goto=' .
                $r[0] .
                '&amp;' .
    ...
    if ($np) {
        $r = db_saq(
            q_limit(
                'SELECT m.id, m.subject FROM fud30_tv_' .
                $frm_id .
                ' tv INNER JOIN fud30_thread t ON tv.thread_id=t.id INNER JOIN fud30_msg m ON t.root_msg_id=m.id WHERE tv.seq IN(' .
                ($id - 10) .
                ', ' .
...
function draw_post_smiley_cntrl()
{
    ...
    $i = 0;
    while ($i < $limit) {
        $smilies .= '<a href="javascript: insertTag(\'txtb\', \'\', \' ' .
            $PS_DST[$i] .
            ' \');">' .
            $PS_SRC[$i++] .
            '</a>&nbsp;';
...
function draw_post_icons($msg_icon)
{
    ...
    foreach ($ICON_L as $k => $f) {
        ...
        $tmp .= '<td class="ac nw"><input type="radio" name="msg_icon" value="' .
            $f .
            '"' .
            ($f == $msg_icon ? ' checked="checked"' : '') .
            ' /><img src="/images/message_icons/' .
            $f .
            '" alt="" /></td>';
    }
    if ($tmp) {
...
function draw_post_attachments($al, $max_as, $max_a, $attach_control_error, $private = 0, $msg_id)
{
    ...
    if (!empty($al)) {
        ...
        $c = uq(
            'SELECT a.id,a.fsize,a.original_name,m.mime_hdr
		FROM fud30_attach a
        ...
        while ($r = db_rowarr($c)) {
            ...
            $insert_uploaded_image = strncasecmp(
                'image/',
                $r[3],
                6
            ) ? '' : '&nbsp;|&nbsp;<a href="javascript: insertTag(\'txtb\', \'[img]/index.php?t=getfile&id=' .
                $r[0] .
                '&private=' .
                $private .
                '\', \'[/img]\');">Insert image into message body</a>';
            $attached_files .= '<tr>
	<td class="RowStyleB">' .
                $r[2] .
                '</td>
	...
	<td class="RowStyleB"><a href="javascript: document.forms[\'post_form\'].file_del_opt.value=\'' .
                $r[0] .
                '\'; document.forms[\'post_form\'].submit();">Delete</a>' .
...
<span class="SmallText">
	...
	<b>Maximum Files Per Message:</b> ' .
        $max_a .
        ($i ? '; currently attached: ' . $i . ' ' . convertPlural($i, ['file', 'files']) : '') .
        '
</span>
' .
...
function smiley_to_post($text)
{
    ...
    foreach (['</pre>' => '<pre>', '</span>' => '<span name="php">'] as $k => $v) {
        $p = 0;
    ...
    foreach ($SML_REPL as $k => $v) {
        ...
        while (($a = strpos($text_l, $k, $a)) !== false) {
            if ((!$a || isset($GLOBALS['__SML_CHR_CHK__'][$text_l[$a - 1]])) &&
                ((@$ch = $text_l[$a + $len]) == '' || isset($GLOBALS['__SML_CHR_CHK__'][$ch]))) {
...
$RSS = ($FUD_OPT_2 &
1048576 ? '<link rel="alternate" type="application/rss+xml" title="Syndicate this forum (XML)" href="/feed.php?mode=m&amp;th=' .
    $th .
    '&amp;basic=1" />
' : '');
...
if (isset($_GET['unread']) && $th && _uid) {
    $_GET['goto'] = q_singleval(
        'SELECT m.id from fud30_msg m LEFT JOIN fud30_read r ON r.thread_id=m.thread_id AND r.user_id=' .
        _uid .
        ' WHERE m.thread_id=' .
        $th .
        ' AND m.apr=1 AND m.post_stamp>CASE WHEN (r.last_view IS NOT NULL OR r.last_view>' .
        $usr->last_read .
        ') THEN r.last_view ELSE ' .
        $usr->last_read .
        ' END'
    );
    if (!$_GET['goto']) {
...
if (!empty($_GET['goto'])) {
    if ($_GET['goto'] === 'end' && $th) {
        [$pos, $mid] = db_saq('SELECT /* USE MASTER */ replies+1,last_post_id FROM fud30_thread WHERE id=' . $th);
        $mid = '#msg_' . $mid;
        $msg_page_focus = 1;
    } else {
        if ($_GET['goto']) { /* Verify that the thread & msg id are valid. */
            ...
            if (!($pos = q_singleval(
                'SELECT /* USE MASTER */ count(*) FROM fud30_msg WHERE thread_id=' .
                $th .
                ' AND id<=' .
                $_GET['goto'] .
                ' AND apr=1'
    ...
    if (!$th) {
        ...
        [$subj, $frm] = array_reverse(explode(':', $_GET['th'], 2));
        $count = q_singleval(
        ...
        if (!$count) {
            ...
            $th = q_singleval(
                q_limit(
                    'SELECT thread_id FROM fud30_msg m
                                        LEFT JOIN fud30_thread t ON m.thread_id = t.id
                                        LEFT JOIN fud30_forum f ON t.forum_id = f.id
                                        WHERE m.subject=' .
                    _esc($subj) .
...
if (_uid) {
    ...
    $first_unread_message_link = (($total - $th) > $count) ? '| <a href="/index.php?t=msg&amp;unread=1&amp;th=' .
        $th .
        '&amp;' .
        _rsid .
        '" title="Go to the 1st unread message inside this topic">Go to unread messages</a>&nbsp;' : '';
    $subscribe_status = $frm->subscribed ? '| <a href="/index.php?t=msg&amp;th=' .
        $th .
        '&amp;notify=' .
        $usr->id .
        '&amp;' .
        ...
        '&amp;SQ=' .
        $GLOBALS['sq'] .
        '" title="Stop receiving notifications about new messages in this topic">Unsubscribe from topic</a>&nbsp;' : '| <a href="/index.php?t=msg&amp;th=' .
        $th .
        '&amp;notify=' .
        $usr->id .
        '&amp;' .
        ...
        '&amp;SQ=' .
        $GLOBALS['sq'] .
        '" title="Receive notification about new messages inside this topic">Subscribe to topic</a>&nbsp;';
    $bookmark_status = $frm->bookmarked ? '| <a href="/index.php?t=msg&amp;th=' .
        $th .
        '&amp;bookmark=' .
        $usr->id .
        '&amp;' .
        ...
        '&amp;SQ=' .
        $GLOBALS['sq'] .
        '" title="Remove this topic from your list of bookmarks">Remove from bookmarks</a>&nbsp;' : '| <a href="/index.php?t=msg&amp;th=' .
        $th .
        '&amp;bookmark=' .
        $usr->id .
        '&amp;' .
        ...
        '&amp;SQ=' .
        $GLOBALS['sq'] .
        '" title="Add this topic to your list of bookmarks">Bookmark topic</a>&nbsp;';
} else {
...
ses_update_status(
    $usr->sid,
    'Browsing topic <a href="/index.php?t=msg&amp;th=' . $frm->id . '">' . $frm->subject . '</a>',
    $frm->forum_id
);
...
function tmpl_create_forum_select($frm_id, $mod)
{
    ...
    if ($mod) { /* Admin optimization. */
        $c = uq(
            'SELECT f.id, f.name, c.id FROM fud30_fc_view v INNER JOIN fud30_forum f ON f.id=v.f INNER JOIN fud30_cat c ON f.cat_id=c.id WHERE f.url_redirect IS NULL ORDER BY v.id'
        );
    } else {
        $c = uq(
            ...
            (_uid ? ' LEFT JOIN fud30_mod mm ON mm.forum_id=f.id AND mm.user_id=' .
                _uid .
                ' LEFT JOIN fud30_group_cache g2 ON g2.user_id=' .
                _uid .
                ' AND g2.resource_id=f.id WHERE mm.id IS NOT NULL OR ' .
                q_bitand('COALESCE(g2.group_cache_opt, g1.group_cache_opt)', 1) .
                ' > 0 ' : ' WHERE ' . q_bitand('g1.group_cache_opt', 1) . ' > 0 AND f.url_redirect IS NULL ') .
            'ORDER BY v.id'
        );
    }
    $f = [$frm_id => 1];
    ...
    while ($r = db_rowarr($c)) {
        if ($oldc != $r[2]) {
            foreach ($GLOBALS['cat_cache'] as $k => $i) {
                ...
                $selection_options .= '<option disabled="disabled">- ' .
        ...
        $selection_options .= '<option value="' .
            $r[0] .
            '"' .
...
function draw_forum_path($cid, $fn = '', $fid = 0, $tn = '')
{
    ...
    $data = '';
    do {
        $data = '&nbsp;&raquo; <a href="/index.php?t=i&amp;cat=' .
            $cid .
            '&amp;' .
            _rsid .
            '">' .
    ...
    return '<a href="/index.php?t=i&amp;' .
        _rsid .
        '">Home</a>' .
        $data .
        ($tn ? '&nbsp;&raquo; <strong>' . $tn . '</strong>' : '');
}
...
if ($use_tmp) {
    q(
        q_limit(
            'CREATE TEMPORARY TABLE fud30__mtmp_' .
            __request_timestamp__ .
            ' AS SELECT id FROM fud30_msg WHERE thread_id=' .
            $th .
            ' AND apr=1 ORDER BY id ASC',
            $count,
            $_GET['start']
        )
    );
...
$q = 'SELECT
    ...
    '
		...
		LEFT JOIN fud30_karma_rate_track karma ON karma.msg_id=m.id AND karma.user_id=' .
    _uid .
    '
		LEFT JOIN fud30_poll p ON m.poll_id=p.id' .
    ($perms & 512 ? ' LEFT JOIN fud30_poll_opt_track pot ON pot.poll_id=p.id AND pot.user_id=' .
        _uid .
        (!_uid ? ' AND pot.ip_addr=' . _esc(get_ip()) : '') : '');
if ($use_tmp) {
    $q .= ' ORDER BY m.id ASC';
} else {
...
while ($obj = db_rowobj($result)) {
    $message_data .= tmpl_drawmsg($obj, $usr, $perms, false, $m_num, [$_GET['start'], $count]);
    $obj2 = $obj;
}
unset($result);
...
if ($FUD_OPT_2 & 32768) {
    $page_pager = tmpl_create_pager(
        $_GET['start'],
        $count,
        $total,
        '/index.php/mv/msg/' . $th . '/0/',
        '/' . reveal_lnk . unignore_tmp . _rsid
    );
} else {
    $page_pager = tmpl_create_pager(
        $_GET['start'],
...
$quick_reply_enabled = _uid &&
    ($GLOBALS['FUD_OPT_3'] & (8388608 | 16777216)) &&
