	...
	if (isset($_POST['btn_submit'])) {
		if (!mail_check()) {
			if (!($email = q_singleval('SELECT email FROM fud30_users WHERE alias='. _esc(htmlspecialchars($_POST['tx_name'])) .' AND '. q_bitand('users_opt', 16) .' > 0'))) {
				error_dialog('ERROR: Unable to e-mail', 'Sorry, the system was unable to e-mail '.htmlspecialchars($_POST['tx_name'], null, null, false).' because it was unable to determine the e-mail address');
			}
...
<tr class="RowStyleB">
	...
	<td class="RowStyleA wa"><input type="text" name="tx_name" value="<?php echo htmlspecialchars($tx_name, null, null, false); ?>" /></td>
</tr>
<tr class="RowStyleB">
	<td class="GenText nw">Subject: <?php echo get_err('tx_subject'); ?></td>
	<td class="RowStyleA wa"><input spellcheck="true" type="text" tabindex="1" name="tx_subject" value="<?php echo (isset($_POST['tx_subject']) ? htmlspecialchars($_POST['tx_subject'], null, null, false).'' : ''); ?>" /></td>
</tr>
<tr class="RowStyleB">
	<td class="GenText nw vt">Body: <?php echo get_err('tx_body'); ?></td>
	<td class="RowStyleA wa"><textarea tabindex="2" name="tx_body" cols="40" rows="20"><?php echo (isset($_POST['tx_body']) ? htmlspecialchars($_POST['tx_body'], null, null, false).'' : ''); ?></textarea></td>
</tr>
<tr class="RowStyleC">
