...
class db
{
    public static $db, $slave, $affected_rows, $res;
}
...
/**
 *
 * @deprecated Moved to database class system
 */
function db_close()
{
    Database::i()->db_close();
...
function &init_user()
{
    ...
    if ($o1 & 128) {
        define('s', $u->ses_id);
        define('_hs', '<input type="hidden" name="S" value="' . s . '" /><input type="hidden" name="SQ" value="' . $sq . '" />');
        if ($o2 & 8192) {
            if ($o2 & 32768) {
        ...
        define('s', '');
        define('_hs', '<input type="hidden" name="SQ" value="' . $sq . '" />');
        if ($o2 & 8192) {
            if ($o2 & 32768) {
...
require($GLOBALS['WWW_ROOT'] . 'Page/' . $t . '.php'); // Bring in the page file
